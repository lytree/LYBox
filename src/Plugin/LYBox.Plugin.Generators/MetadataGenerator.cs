...
public class MetadataGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(generationInputs, static (sourceContext, data) =>
        {
            ...
            // 双轨收敛：仅 emit 元数据 + Web 描述符 + 图标资源加载器。
            // UI（视图/导航/菜单）由 IGeneratedPluginModule.Ui 描述符单轨承载（{ClassName}.Module.g.cs）。
            var source = $@"// <auto-generated />
#nullable enable
using LYBox.Plugin.Shared;
{webUsing}{iconResourcesUsing}
namespace {target.Namespace}
{{
    public partial class {target.ClassName} : {baseInterfaces}
    {{
{metaLines}
{webLines}{iconResourcesLines}    }}
}}";
